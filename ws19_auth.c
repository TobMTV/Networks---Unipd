...
printf("===== HEADERS ==============\n"); // Mi estrapolo method path e ver
for(i=1;i<k;i++){
	printf("%s --> %s\n", h[i].n, h[i].v);	
}
method = h[0].n;
...
printf("\n%s %s %s\n",method,path,ver);
for(i=0;i<k;i++){
	if (!strcmp(h[i].n, "Authorization")){ // se il method e' auth significa che il client vuole autenticarsi con le credenziali successive a esso
		scheme = &h[i].v[1];
		for(j=1; h[i].v[j]!=' '; j++){}
		h[i].v[j]=0;
		credent = &h[i].v[++j];
